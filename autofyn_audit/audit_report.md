...
A developer discovers a suspicious third-party package (`chain3-shadow`) in their dependency tree. They do the right thing: they add `allowBuilds: {chain3-shadow: false}` to `pnpm-workspace.yaml` to block its lifecycle scripts. An attacker who can modify the project's `pnpm-lock.yaml` exploits two independent vulnerabilities to steal credentials anyway — in a single `pnpm install`:
...
2. **VULN-5 — Bin entries bypass the allowBuilds block:** Despite the block, chain3-shadow declares `"bin": {"node": "./payload.sh"}` in its `package.json`. The bin linking code at `link-bins/src/index.ts` is not guarded by the allowBuilds check that protects lifecycle scripts. pnpm links `payload.sh` into `node_modules/.bin/node` regardless of the policy. The malicious binary is now on PATH — silently, with no warning.
...
- **VULN-5** (Medium, 6.3): `link-bins/src/index.ts` links a package's `bin` entries into `node_modules/.bin/` without checking whether the package is blocked by `allowBuilds`. A package whose lifecycle scripts are blocked can still inject arbitrary executables onto PATH.
...
**Critical.** The developer did everything right — identified the suspicious package, used `allowBuilds` to block it, and explicitly allowed only the trusted package's lifecycle. The policy is incomplete at two points: bin entries bypass the block (VULN-5), and the lockfile enables loading secrets from disk without any validation (VULN-9). The attack requires only lockfile control (feasible via PR, compromised CI, or supply chain compromise) and relies on the developer's own security decision (`allowBuilds`) to create the false confidence that makes the attack viable. Standard code review is unlikely to catch the traversal path in `resolution.directory` entries or notice that a blocked package's `bin` entries remain active.
